...
func StartCRCWithDefaultBundleSucceedsOrFails(expected string) error {
	...
	crcStart := crcCmd.Command{Action: "start", DisableUpdateCheck: true}.ToString()
...
func StartCRCWithDefaultBundleWithStopNetworkTimeSynchronizationSucceedsOrFails(expected string) error {
	...
	crcStart := crcCmd.Command{Action: "start", DisableUpdateCheck: true, DisableNTP: true}.ToString()
	cmd = fmt.Sprintf("%s -p '%s' %s --log-level debug", crcStart, pullSecretFile, extraBundleArgs)
	err := clicumber.ExecuteCommandSucceedsOrFails(cmd, expected)
